...
def flash_laser(color):
    laser = Canvas(root, width=520, height=470, highlightthickness=0, bg=bg_color)
    laser.place(x=0, y=0)
    line = laser.create_rectangle(0, 470, 520, 450, fill=color, width=0)

    def animate():
        for y in range(470, 0, -5):
            laser.coords(line, 0, y, 520, y - 20)
            time.sleep(0.005)
        laser.destroy()
...
def decrypt_file():
    while True:
        ...
        try:
            ...
            flash_laser("#ff6f61")
            messagebox.showerror("Decryption Failed 💥", "🔴 Decryption failed! Incorrect key hidden in image.\nPlease try again.")
        except Exception as e:
            flash_laser("#ff6f61")
...
Label(root, text="🔐 StegoSecure: AES + Steganography 🔐", font=title_font, bg=bg_color, fg="#7b4397").pack(pady=30)

Button(root, text="🔒 Encrypt File + Hide Key", command=encrypt_file,
       bg=btn_encrypt, fg="white", font=btn_font, relief="raised",
       activebackground="#c089ff", width=33, bd=3).pack(pady=20)

Button(root, text="🔓 Decrypt File Using Image Key", command=decrypt_file,
       bg=btn_decrypt, fg="white", font=btn_font, relief="raised",
       activebackground="#82cfff", width=33, bd=3).pack(pady=10)

Label(root, text="✨ Created by Anjali ✨", font=("Segoe UI", 9), bg=bg_color, fg="#999").pack(side="bottom", pady=15)
